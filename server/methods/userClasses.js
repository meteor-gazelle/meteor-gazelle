Meteor.methods({
  updateUserClass: function (modifier, documentId) {
    ...
    var newUserClass = Class.findOne({
      _id: documentId
    });
    UserClass.update({
      classId: documentId
  ...
  editUserClasses: function (doc) {
    ...
    doc.classes.forEach(function (element) {
      ...
      var classDoc = UserClass.findOne({
        classId: classId
      });
      if (!classDoc) {
      ...
      Roles.addUsersToRoles(doc.userId, Class.findOne({
        _id: classDoc.classId
      }, {
        roles: 1
      }).roles, 'class');
    });
