function validatePermissions (permissions) {
  ...
  if (_.without(permissions, this.permissions).length > 0) {
    throw new Meteor.Error('invalid-argument',
      'An invalid permission was supplied');
  }
...
const PermissionGroup = class PermissionGroup {
  ...
  _addPermissions (permissions) {
    ...
    permissions.forEach(permission => {
      ...
      if (!permission.hasOwnProperty('title') ||
        !permission.hasOwnProperty('description')) {
        throw new Meteor.Error('invalid-permission-format',
          `A permission provided to the
          ${this.title} group is missing a title and/or description`);
      }
      // Check that the title and description are strings
      check(permission.title, String);
      check(permission.description, String);
      // If the permission is a duplicate (already been added to the group)
      // throw an exception
      if (_.contains(this.addedPermissions, permission.title)) {
        throw new Meteor.Error(`Permission ${permission.title} already in
        group ${this.title}`);
      }
      // Add the permission
