...
function run_tests {
  ...
  # which velocity > /dev/null || fail "velocity command not found; run 'npm install -g velocity' to install velocity"
  [ -f settings.json ] || fail "settings.json not found; run 'cp settings.json.template settings.json'"

  # do the thing
  velocity test-package ./packages/* --settings settings.json --ci
}
